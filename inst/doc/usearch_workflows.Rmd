...
The `biom` file containing only the OTU table can be read in with `phyloseq`'s `import_biom` function provided a modified `parseFunction` is given. The `USEARCH` taxonomy files should be broken on commas.    
