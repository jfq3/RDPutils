...
The RDP classifier is provided with training sets for the classification of bacterial and archael 16S rRNA and fungal 28S rRNA and ITS gene sequences, and can classify multiple samples at one time. An example command is:  

```
java -Xmx2g -jar <path_to>/RDPTools/classifier.jar classify -g fungallsu
  -c 0.5 -f fixrank -h test_hier.txt c:/RDP_examples/*.fasta
```
In this example, fungal 28S rRNA  gene sequences in directory c:/RDP_examples/ are classified with confidence 0.5. The format is fixrank, meaning that only the ranks Domain, Phylum, Class, Order, Family, and Genus will be included in the classification. There will not be any sub-classes, etc., nor any missing ranks. Results are written to the file test_hier.txt in "hierachical" format. These results are imported into `phyloseq` with the `RDPutils` function `hier2phylseq`:  
