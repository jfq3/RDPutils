#' @title Import USEARCH otutab_taxa File into phyloseq
#' @name import_otutab_taxa
#' @aliases import_otutab_taxa
#' @description Converts USEARCH's usearch_global output option otutabout to a phyloseq object with otu_table and tax_table.
#' @usage import_otutab_taxa(in_file)
#' @param in_file An otutab_taxa file generated by USEARCH 8.1+
#' @details The USEARCH usearch_global function has an option to output an OTU table with taxonomy assignments in a tab-delimited text file. The output resembles the legacy QIIME format. This function imports such a file into a phyloseq object with otu_table and tax_table.
...
import_otutab_taxa <- function(in_file) {
  ...
  if ("taxonomy" %in% colnames(temp)) {
    ...
    expt <- phyloseq::otu_table(temp, taxa_are_rows = TRUE, errorIfNULL = TRUE)
  }
